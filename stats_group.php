...
$url = '/blocks/exastats/stats_group.php';
$PAGE->set_url($url, ['courseid' => $courseid]);

require_login($courseid);

$userRole = block_exastats_get_role_byusername();
// only for teachers and director!!
...
$PAGE->set_title(get_string('page_stats_group_title', 'block_exastats'));
...
if (!isset($filter['schoolnumber'])) {
    ...
    $schoolid = intval($filter['schoolnumber']);
}

echo '<h2>'.get_string('group_stats', 'block_exastats').'</h2>';
// filter form
echo '<form method="post" class="form-inline">';
echo '<div class="form-group">
        <label class="sr-only">'.get_string('form_schoolnumber_label', "block_exastats").'</label>
        <input type="text" class="form-control" name="filter[schoolnumber]" value="'.$schoolid.'" placeholder="'.get_string('form_schoolnumber_label', "block_exastats").'"/>
    </div>
    <div class="form-group">
        <label class="sr-only">'.get_string('form_group_label', "block_exastats").'</label>
        <input type="text" class="form-control" name="filter[group]" value="'.(isset($filter['group']) ? $filter['group'] : '').'" placeholder="'.get_string('form_group_label', "block_exastats").'"/>    
    </div>
    <button type="submit" class="btn btn-default">'.get_string('button_show', "block_exastats").'</button>
</form>';
echo '<div class="bloks_exastats group_stats">';

if ($schoolid == 999999) {
    return block_exastats_stopOutputWithMessage($OUTPUT, get_string('fillSchoolnumberAndGroup', 'block_exastats'), $courseid);
}

if (!($schoolid > 0) || !isset($filter['group']) || trim($filter['group']) == '') {
    return block_exastats_stopOutputWithMessage($OUTPUT, get_string('fillSchoolnumberAndGroup', 'block_exastats'), $courseid);
}
...
if ($filter['group']) {
    $addFilters['group'] = trim($filter['group']);
}
$users = block_exastats_get_groupusers($schoolid, true, $addFilters);
